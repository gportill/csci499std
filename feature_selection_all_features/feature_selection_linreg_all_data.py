...
def make_df(year):
    ...
    file = "../all_features_dfs/" + str(year) + "selected_features_cpp.xlsx"

    full_df.to_excel(file, na_rep="nan")


def create_model(year, yearTrain=2006):
    if year < 2006 or year > 2011:
        print("year must be [2006, 2011]")
        return

    file = '../all_features_dfs/' + str(year) + 'all_features_cpp.xlsx'
    fileTrain = '../all_features_dfs/' + str(yearTrain) + 'all_features_cpp.xlsx'

    full_df = pd.read_excel(file, dtype=str)
    full_df = full_df.astype(float)
    full_df = full_df.drop('fips', axis=1)

    full_df_train = pd.read_excel(fileTrain, dtype=str)
    full_df_train = full_df_train.astype(float)
    full_df_train = full_df_train.drop('fips', axis=1)

    x = full_df.drop('target_t5', axis=1)
    y = full_df.target_t5
    x_train = full_df_train.drop('target_t5', axis=1)
    y_train = full_df_train.target_t5
    ...
    # --------- Linear Regression for Test/Train Different Years ---------------
    ...
    # pred_test = lm.predict(x_test)
    #
    # coefs_df = pd.DataFrame(zip(x_test.columns, lm.coef_), columns=['features', 'estimated coefficients'])
    # print(coefs_df)
    #
    # r2 = sklearn.metrics.r2_score(y_test, pred_test)
    # print("r2: " + str(r2))
...
# first year is test year, second year is training year
create_model(2008, 2007)
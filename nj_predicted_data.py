...
def create_model(year):
    ...
    nj_fips = list(std_clinics_per_county.keys())
    # print(nj_fips)
    ...
    # make copy of data frame with only new jersey rows
    nj_data = full_df.loc[nj_fips, :].copy()
    # should drop target_t5
    nj_data.drop('target_t5', axis=1, inplace=True)
    print(nj_data)
    print(nj_data.shape)
    ...
    #print("printing pred test")
    # print(pred_test)

    # keep top 20 features in nj_data
    for f in list(remove_features.index):
        nj_data = nj_data.drop([f], axis=1)

    nj_pred = lm.predict(nj_data)
    nj_fips_and_pred = zip(nj_fips, nj_pred)
    nj_df = pd.DataFrame(nj_fips_and_pred)

    file = "nj_predictions_" + str(year) + ".xlsx"

    nj_df.to_excel(file, index=False)





# the data returned will be: if we are in 2015 right now, what will STD rates for 2016 be?
create_model(2011)

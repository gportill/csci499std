# predict cases (normalized) with engineered features
...
def create_model(year):
    ...
    print("column names: " + str(list(full_df.columns.values)))

    # -------- begin linear regression --------
    full_df = full_df.dropna()  # remove any rws with NaN values
    ...
    feature_coefficients_df = feature_coefficients_df.reindex(
        feature_coefficients_df.median(axis=1).abs().sort_values().index)
